...
from . import functional as QMCF
...
class QMCTransforms:
    ...
    def __init__(self,
        ...
        self.qmc_quotas = qmc_quotas

    def __call__(self, img, x):
        index = 0
        for _, (t, q) in enumerate(zip(self.transforms, self.qmc_quotas)):
            if q != 0:
                sub_x = x[index:index + q]
                index += q
                img = t(img, sub_x)
            else:
                img = t(img)
...
class RandomCrop(torch.nn.Module):
    ...
    @staticmethod
    def get_params(img: Tensor, output_size: Tuple[int, int], x) -> Tuple[int, int, int, int]:
        ...
        i = unit_interval_to_categorical(x[0], h - th + 1)
        j = unit_interval_to_categorical(x[1], w - tw + 1)
        return i, j, th, tw
    ...
    def forward(self, img, x):
        ...
        width, height = QMCF.get_image_size(img)
        ...
        i, j, h, w = self.get_params(img, self.size, x)
...
class RandomPerspective(torch.nn.Module):
    ...
    def forward(self, img, x):
        ...
        if x[0] < self.p:
            width, height = QMCF.get_image_size(img)
            startpoints, endpoints = self.get_params(width, height, self.distortion_scale, x)
    ...
    @staticmethod
    def get_params(width: int, height: int, distortion_scale: float, x) -> Tuple[List[List[int]], List[List[int]]]:
        ...
        half_width = width // 2
        topleft = [
            unit_interval_to_categorical(x[1], int(distortion_scale * half_width) + 1),
            unit_interval_to_categorical(x[2], int(distortion_scale * half_height) + 1)
        ]
        topright = [
            -unit_interval_to_categorical(x[3], int(distortion_scale * half_width) + 1) - 1 + width,
            unit_interval_to_categorical(x[4], int(distortion_scale * half_height) + 1)
        ]
        botright = [
            -unit_interval_to_categorical(x[5], int(distortion_scale * half_width) + 1) - 1 + width,
            -unit_interval_to_categorical(x[6], int(distortion_scale * half_height) + 1) - 1 + height
        ]
        botleft = [
            unit_interval_to_categorical(x[3], int(distortion_scale * half_width) + 1),
            -unit_interval_to_categorical(x[4], int(distortion_scale * half_height) + 1) - 1 + height
        ]
...
class RandomResizedCrop(torch.nn.Module):
    ...
    @staticmethod
    def get_params(img: Tensor, scale: List[float], ratio: List[float], x) -> Tuple[int, int, int, int]:
    ...
    def forward(self, img, x):
        ...
        """
        i, j, h, w = self.get_params(img, self.scale, self.ratio, x)
...
class RandomRotation(torch.nn.Module):
    ...
    @staticmethod
    def get_params(degrees: List[float], x) -> float:
        """Get parameters for ``rotate`` for a random rotation.

        Returns:
            float: angle parameter to be passed to ``rotate`` for random rotation.
        """
        angle = float(degrees[0]) + x[0] * (float(degrees[1]) - float(degrees[0]))
        return angle
    ...
    def forward(self, img, x):
        ...
        if isinstance(img, Tensor):
            if isinstance(fill, (int, float)):
                ...
                fill = [float(f) for f in fill]
        angle = self.get_params(self.degrees, x)
...
class RandomAffine(torch.nn.Module):
    ...
    def get_params(
        ...
        if translate is not None:
            ...
            tx = int(round(-max_dx + x[0] * (max_dx + max_dx)))
            ty = int(round(-max_dy + x[1] * (max_dy + max_dy)))
            translations = (tx, ty)
        else:
            translations = (0, 0)

        if scale_ranges is not None:
            scale = float(scale_ranges[0] + x[2] * (scale_ranges[1] - scale_ranges[0]))
        else:
            scale = 1.0

        shear_x = shear_y = 0.0
        if shears is not None:
            shear_x = float(shears[0] + x[3] * (shears[1] - shears[0]))
            if len(shears) == 4:
                shear_y = float(shears[2] + x[4] * (shears[3] - shears[2]))
    ...
    def forward(self, img, x):
        ...
        img_size = QMCF.get_image_size(img)

        ret = self.get_params(self.degrees, self.translate, self.scale, self.shear, img_size, x)
...
class RandomErasing(torch.nn.Module):
    ...
    @staticmethod
    def get_params(
        x, img: Tensor, scale: Tuple[float, float], ratio: Tuple[float, float], value: Optional[List[float]] = None,
        ...
        log_ratio = torch.log(torch.tensor(ratio))
        for _ in range(10):
            erase_area = area * (scale[0] + x[1] * (scale[1] - scale[0]))
            aspect_ratio = torch.exp(log_ratio[0] + x[2] * (log_ratio[1] - log_ratio[0]))
            ...
            i = unit_interval_to_categorical(x[3], img_h - h + 1)
            j = unit_interval_to_categorical(x[4], img_w - w + 1)
            return i, j, h, w, v
    ...
    def forward(self, img, x):
        ...
        if x[0] < self.p:
            ...
            x, y, h, w, v = self.get_params(x, img, scale=self.scale, ratio=self.ratio, value=value)
...
class GaussianBlur(torch.nn.Module):
    ...
    @staticmethod
    def get_params(sigma_min: float, sigma_max: float, x) -> float:
        ...
        """
        return sigma_min + x[0] * (sigma_max - sigma_min)

    def forward(self, img: Tensor, x) -> Tensor:
        """
        Args:
        ...
        """
        sigma = self.get_params(self.sigma[0], self.sigma[1], x)
