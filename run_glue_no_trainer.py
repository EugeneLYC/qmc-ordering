...
from qmcordering import build_sorter
...
def main():
    ...
    if args.shuffle_type in ['ZO']:
        dimension = 0
        for n, p in model.named_parameters():
            if len(n.split('.')) >= 4 and n.split('.')[3] <= '5' and n.split('.')[3] >= '0':
                dimension += p.numel()
    ...
    # for n, p in model.named_parameters():
    #     if len(n.split('.')) >= 4 and n.split('.')[3] <= '5' and n.split('.')[3] >= '0':
    #         t = torch.normal(0, 0.02*0.02, p.data.shape, requires_grad=False, device=p.device)
    #         p.data.zero_().add_(t)
